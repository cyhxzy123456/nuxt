...
export function WangGuan(pay_money) {
  const result = get('/api/PaymentControll/Xpay_NetPay',
    {pay_money:pay_money,return_uri:'https://www.hefeng178.com/personCenter/wangGuan'});
  return result;
}
//找回提现密码
...
export function GetAgreement(borrowId,contractType) {
  const result = post('/api/UserContract/Index',
    {borrowId:borrowId,contractType:contractType,returnUrl:"https://www.hefeng178.com/agreen"});
  return result;
}
...
//查询意见看是否有未读
export function GetFeedBackNo(pagesize,pageindex) {
  const result = post('/api/wWorkOrder/Index',
    {
      "PageSize":pagesize,
      "PageIndex":pageindex,
