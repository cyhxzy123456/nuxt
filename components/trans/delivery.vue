    ...
    <div>
      <table class="list-table">
        ...
        <tr v-for="(item,index) in deliveryList" :key="index">
          <td v-text="item.followid"></td><td v-text="">{{item.time_deal | dateString('YYYY-MM-DD')}}</td><td >{{item.time_deal}}</td>
...
<script>
  import {DeliverInfo} from '../../plugins/myTrade/myTrade'//获取交割单
    export default {
      name: "delivery",
      props:['id'],
      data(){
