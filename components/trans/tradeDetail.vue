    ...
    <div class="tradeDetail">
      ...
      <Pop :isShow="ShowGetProfit" @closePop="close('ShowGetProfit')" >
        ...
        <div class="addBound-balance">
          策略可用余额：<span class="red">{{tradeDetail.moneyBalanceAble}}</span> 元
          <br>
        ...
        <div class="addBound-tip">最高可提利润 <span class="yellow" v-text="canProfit">0</span>元</div>
        <div class="btnGroup">
      ...
      <Pop :isShow="showSettle" @closePop="close('showSettle')">
        <h3 slot="tit">结算确认</h3>
        <p class="tip">您的合约已使用 <span v-text="all.dealDays<0?0:all.countday"></span>天({{all.dealDays}}个交易日)，<span v-show="type==12">当前还剩余{{all.remainNumber}}天，相关费用概不退还，</span>是否确认结算？</p>
        <div class="btnGroup">
...
<script>
  import {ProductDetail} from '../../plugins/myTrade/myTrade'//获取合约详情的状态
  import {GetProfit} from '../../plugins/myTrade/myTrade'//提取利润
  import {AddContract} from '../../plugins/myTrade/myTrade'//追加操盘
  import {GetAddMoney} from '../../plugins/myTrade/myTrade'//追加保证金
  import {Settle} from '../../plugins/myTrade/myTrade'//结算
    ...
    export default {
      ...
      methods:{
        ...
        commitProfit(){
          ...
          if(this.drawingMoney > this.tradeDetail.moneyProfit){
...
<style  rel="stylesheet/less" lang="less" scoped>
  .addBound-tip{padding:10px 0;background: url("../../assets/img/trade/tan.png") no-repeat left center;padding-left:28px;font-size: 14px;
    color: red;}
.tradeDetail{font-size: 16px;
  .deal-title1 {position: absolute;right:20px;top:120px;z-index: 1;
    ...
    .clock {display: inline-block;width: 15px;height: 15px;background: url(../../assets/img/trade/clock.png) 100% 100% no-repeat;background-size: 100%;margin-right: 0px;margin-left: 30px;}
  ...
  .powerline {display: block;width: 1172px;padding-top: 10px;padding-bottom: 10px;border: 1px solid #e5e5e5;border-top: none;margin-bottom: 30px;
    .buoys{background: url("../../assets/img/trade/buoys.gif") no-repeat;margin-left:664px;color: #FFF;
