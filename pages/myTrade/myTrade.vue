            ...
            <div>
                ...
                <ul class="trading" >
                  <li v-for="(item,index) in MyTradeList" :key="index" v-cloak class="item">
                    <div class="item-head">
                      ...
                      <span class="fr" >{{item.date_begin|dateString('YYYY-MM-DD')}} <i v-show="item.product_type!=31&&item.product_type!=13">～{{item.date_end | dateString('YYYY-MM-DD')}}</i></span>
                    </div>
                    <div class="item-cont">
                      <div  class="fl left">
                        <div v-show="item.product_type!=31">
                          <img  src="../../assets/img/trade/myTrade-bg.png" alt="">
                          <ul >
              ...
              <div  v-show="statusShow==23">
                ...
                <ul class="sell_ul1 settle">
                  <nuxt-link tag="li" :to="{name:'myTrade-hisTradeDetail-id',params:{id:item.borrowid}}" style="cursor: pointer" v-for="(item,index) in MyTradeListSettle" :key="index">
                    <div>
                      <div class="item-head">
                        <span>{{item.product_name}}</span><span>({{item.id}})</span>
                        <span class="fr">{{item.date_begin|dateString('YYYY-MM-DD')}}～{{item.date_end | dateString('YYYY-MM-DD')}}</span>
                      </div>
                      <ul class="clearfix">
...
<script>
  ...
  import {ProductList} from '../../plugins/myTrade/myTrade'
  import {GetAgreement} from '../../plugins/personCenterApi/personCenter'
  import {SeeAgreement} from '../../plugins/personCenterApi/personCenter'
  import {DownAgreement} from '../../plugins/personCenterApi/personCenter'
      ...
      goTrade(type){
        if(this.nowType==10){
          ...
          if(this.nowType==type){
            this.$router.push({path:'/trade/trade',query:{type:type}})
          }
        }
      },
      //历史合约进入策略中
...
<style  rel="stylesheet/less" lang="less" scoped>
  ...
  .settle>li{background: url("../../assets/img/trade/mt-settled.jpg") no-repeat right bottom;}
