        ...
        <div class="content" >
          <ul class="news-list clearfix">
            <li v-for="(item,index) in list" :key="index">
              <nuxt-link :to="{name:'news-newsDetail',params:{id:item.id}}">
               <!-- <div class="date">
                  <b>{{item.timeAdd | dateString('MM-DD')}}</b>
                  <span>{{item.timeAdd | dateString('YYYY')}}</span>
                </div>
                <div class="con">
                  <h4 v-text="item.title">5月13日贝赢微资讯</h4>
                  <p v-text="item.sketch">今日关注：1、中国证券报：上交所与中国结算于5月12日（周六）组织科创板业务第一次全网测试。券商在测试中较为关注科创板集中交易及两融业务系统测试，而且此次测试并不包括科创板转融券业务系统。目前暂时仅有9家头部券商获取科创板转融券业务权限并已展开相关测试。2、中国…</p>
                </div>-->
...
<style rel="stylesheet/less" lang="less" scoped>
  .news-list{
    .news-banner {background: url(../../assets/img/news/news-banner.jpg) center no-repeat;height: 400px;width: 100%;}
