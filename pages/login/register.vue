    ...
    <div class="login-box" >
      <div class="wrap clearfix" >
        <div class="login-content fl">
          <h2 class="login-tit"><nuxt-link to="/login/register" class="go-reg">注册</nuxt-link></h2>
          <div class="login-inp">
            <label>
          ...
          <input type="text"  placeholder="设置昵称" class="form-input" v-model="nickName">
            ...
            <div class="reg_btn_box">
              ...
              <div style="margin-top:10px"><button type="button" class="btn-blue" :disabled="able" @click="register" >注册</button></div>
            </div>
          </div>
        </div>
        <div class="fl login-btn">
          <p>已有账号？ </p>
          <p>立即登录！</p>
          <nuxt-link to="/login/login">立即登录</nuxt-link>
        </div>
      </div>
...
<script>
  ...
  import {reg} from '../../plugins/loginApi/login'
  import {getProtocol} from '../../plugins/globalApi/global'
    export default {
      name: "register",
      ...
      methods:{
        ...
        register(){
          ...
          if(this.$store.getters.config.isRequireRecommend==0){
...
<style rel="stylesheet/less" lang="less" scoped>
  ...
  .login-box {background: url(../../assets/img/login/login-bg.png)no-repeat center;padding: 50px 0;
