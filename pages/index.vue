<template>
  <div class="content_main">
    ...
    <div class="noticeBox">
      <div class="wrap">
        <div class="notice">
          ...
          <nuxt-link to="/personCenter/message" class="fr" tag="span">更多 >></nuxt-link>
        </div>
      </div>
    </div>
          ...
          <div class="top-cont">
            <ul class="clearfix">
              <li>
                ...
                <img src="../assets/img/index/product1.png" alt="product1">
              ...
              <li>
                <h3>资产委托人</h3>
                <img src="../assets/img/index/product3.png" alt="product3">
                <p>合法拥有投资资金，把投资资金存放在证券账户中作为委托资产。</p>
              </li>
            </ul>
          </div>
          <div class="bot-cont clearfix">
            <img src="../assets/img/index/product4.png" alt="product4" calss="fl">
            <div class="fr text">
              <p>合丰网平台提供证券信息的撮合服务、风控技术服务、客服服务，为策略建议人与资产委托人提供一个优质的信息服务平台，上线至今为客户提供了全面的服务，体现了合丰网平台的专业水准，获得了客户的一致好评，合丰网汇集了优秀的金融界精英， 公司凭借多年金融行业资产配置经验、以客户为中心，秉承专业、诚信、贴心、共赢的服务宗旨，以诚信共赢、高效专业的经营理念，赢得了客户及合作伙伴的高度认可，紧密结合金融市场环境及用户需求，不断加强创新能力，提高企业综合实力，致力于为用户打造一个安全、专业、可信赖的信息服务平台，让人人都懂得合理资产配置，实现财富增值。</p>
              ...
              <div class="go">
                <nuxt-link to="/trade/trade" tag="button">申请策略</nuxt-link>
              </div>
            </div>
          </div>
        </div>
            ...
            <ul ref="slide1">
              <li class="item" v-for="(item,index) in dealList" :key="index">
                <ul class="item-line" >
                  ...
                  <li>
                    <p>交易类型</p>
                    <p v-text="item.deal_type==0?'':'卖出策略'"></p>
                  </li>
                </ul>
              </li>
            </ul>
    ...
    <div class="step cont bg-white">
      <div class="wrap">
        <div class="step-cont">
          <nuxt-link  to="/trade/trade">申请策略</nuxt-link>
          <nuxt-link to="/">撮合资产委托人</nuxt-link>
          <nuxt-link to="/">同意《委托投资协议》</nuxt-link>
          <nuxt-link to="/">支付策略风险保证金</nuxt-link>
          <nuxt-link to="/">提供证券投资策略建议信息</nuxt-link>
          <nuxt-link to="/">结算策略盈亏</nuxt-link>
          <nuxt-link to="/">终止策略</nuxt-link>
        </div>
      </div>
    </div>
        ...
        <div class="news-cont">
          <ul class="clearfix">
            ...
            <li class="deep-gray list">
              <h3>{{nav1}}</h3>
              <p v-for="(item,index) in list1" :key="index"><nuxt-link tag="span" :to="{name:'news-newsDetail',params:{id:item.id}}">{{item.sketch}}</nuxt-link><span>{{item.timeAdd | dateString('YYYY.MM.DD')}}</span></p>
              <i class="jian"></i>
            </li>
            <li><img src="../assets/img/index/news2.png" alt=""></li>
            <li>NEWS</li>
            <li>NEWS</li>
            <li class="white-bg list">
              <h3>{{nav2}}</h3>
              <p v-for="(item,index) in list2" :key="index"><nuxt-link tag="span" :to="{name:'news-newsDetail',params:{id:item.id}}">{{item.sketch}}</nuxt-link><span>{{item.timeAdd | dateString('YYYY.MM.DD')}}</span></p>
              <i class="jian"></i>
            </li>
            <li><img src="../assets/img/index/news3.png" alt=""></li>
            <li class="white-bg list">
              <h3>{{nav3}}</h3>
              <p v-for="(item,index) in list3" :key="index"><nuxt-link tag="span" :to="{name:'news-newsDetail',params:{id:item.id}}">{{item.sketch}}</nuxt-link><span>{{item.timeAdd | dateString('YYYY.MM.DD')}}</span></p>
              <i class="jian"></i>
            </li>
            <li><nuxt-link tag="button" to="/about/newsList">更多资讯</nuxt-link></li>
          </ul>
        </div>
      </div>
    </div>
...
<script type="text/ecmascript-6">
  ...
  import {getBanner} from '../plugins/globalApi/global'
  import {getList} from '../plugins/newApi/news'
  import {GetChild} from '../plugins/newApi/news'
  import {GetTrade} from '../plugins/myTrade/myTrade'//最新交易
  import {GetLastProfit} from '../plugins/myTrade/myTrade'//最新盈利
...
<style rel="stylesheet/less" lang="less" scoped>
  ...
  .noticeBox{height:40px;line-height:40px;background: #eaeaea;overflow: hidden;font-size: 14px;
    ...
    span:first-child{margin-right: 85px;padding-left:27px;background: url("../assets/img/index/laba.png") no-repeat left center;background-size:21%}
    span:last-child{cursor: pointer}
  }
  .headTit{font-weight: bold;text-align: center;padding-top:60px;position: relative;
    .lineTit{    font-size: 14px;height: 64px;background: url(../assets/img/index/line.png) center no-repeat;padding: 0;}
  ...
  .product{background: #fff;
    ...
    .product-cont{
      .top-cont{margin-bottom: 60px;
        ...
        li.active{background: url("../assets/img/index/product2.png") no-repeat;height:355px;background-size: cover;color:#fff;
          h3{color:#fff}
        }
      }
      .bot-cont{
        .text{width:70%}
        p{line-height: 36px;color:#333;text-indent: 2em;padding-bottom: 15px}
  ...
  .step{
    .step-cont{background: url("../assets/img/index/step-bg.png") no-repeat;height:315px;position: relative;
      a{position: absolute}
  ...
  .news{font-size: 14px;background: #F2F0EF;overflow: hidden;padding-bottom: 70px;
    ...
    .deep-gray{    background: #2d2d2d;
      .jian{background: url("../assets/img/index/jian-w.png") no-repeat}
    }
    .white-bg{background: #fff;
      p{color:#666}
      h3{color:#333!important;}
      .jian{background: url("../assets/img/index/jian.png") no-repeat}
    }
    .title{color:#fff;
      p{color:#fff;}
