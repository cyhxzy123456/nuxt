        ...
        <ul>
          <li v-for="(item,index) in backList" :key="index">
            <div>
              ...
              <p class="fr detail"><span >{{item.addtime}}</span>
                <nuxt-link tag="button" :to="{name:'personCenter-feedbackDetail',params:{id:item.id}}">查看详情</nuxt-link>
                <span class="dian" v-show="item.notreadcount>0"></span>
              </p>
            </div>
            <div>
...
<script>
  import {CommitFeedBack} from '../../plugins/personCenterApi/personCenter'
  import {GetFeedBack} from '../../plugins/personCenterApi/personCenter'
  import {OverFeedBack} from '../../plugins/personCenterApi/personCenter'
  import Pop from '../../components/pop/pop'
  import ImgVCode from '../../components/code/ImgVCode'
  import {GetManage} from '../../plugins/personCenterApi/personCenter'//获取后台回复
    export default {
      name: "feedback",
      layout:'left',
...
<style rel="stylesheet/less" lang="less" scoped>
  .dian{width:10px;height:10px;background: red;border-radius: 50%;display: inline-block;    margin-left: -20px;}
  ...
  .feedback{width:100%;padding:35px;
    ...
    .cont2{
      ...
      button{margin-left:30px;padding:0 20px}
