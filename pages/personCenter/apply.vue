<template>
    <div class="apply center-right">
      <ul>
        <li v-show="day!=''"><span>{{day}}</span><span>资产委托合作期限：最少2个交易日，自动展期</span><nuxt-link tag="button" to="/trade/trade">去创建{{day}}</nuxt-link></li>
        <li v-show="month!=''"><span>{{month}}</span><span>资产委托合作期限：1自然月，自动展期</span><nuxt-link tag="button" :to="{path:'/trade/trade',query:{type:12}}">去创建{{month}}</nuxt-link></li>
        <li v-show="ke!=''"><span>{{ke}}</span><span class="two">资产委托合作期限：至少2个交易日，自动展期；按购买市值收取费用，策略灵活配置</span><nuxt-link tag="button" :to="{path:'/trade/trade',query:{type:13}}">去创建{{ke}}</nuxt-link></li>
        <li v-show="hui!=''"><span>{{hui}}</span><span>策略约定可用3个交易日，盈利7成归您</span><nuxt-link tag="button" :to="{path:'/trade/trade',query:{type:15}}">去创建{{hui}}</nuxt-link></li>
        <li v-show="mom!=''"><span>{{mom}}</span><span class="two">资产委托合作期限：最少2个交易日，自动展期<br> {{remark[1]}}</span><nuxt-link tag="button" :to="{path:'/trade/trade',query:{type:31}}">去创建{{mom}}</nuxt-link></li>
      </ul>
    </div>
</template>
<script>
